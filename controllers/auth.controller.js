...
const { generateVerificationEmail, generateForgotPasswordEmail } = require('../utils/emailTemplates');
...
const register = async (req, res) => {
    try {
        ...
        sendEmail(generateVerificationEmail({
            fullname: newUser.fullname,
            code: codeVerify
        }));
...
const verifyCode = async (req, res) => {
    try {
        ...
        if (user.codeVerify !== codeVerify || Date.now() - user.timeSendCode.getTime() > 5 * 60 * 1000) {
...
const forgotPassword = async (req, res) => {
    try {
        ...
        await user.save();
        sendEmail(generateForgotPasswordEmail({
            fullname: user.fullname,
            code: codeVerify
        }));
...
const updateProfile = async (req, res) => {
    try {
        ...
        if (!userId || !fullname || !username || !email) {
            return res.status(400).json({ message: 'User ID, fullname, username, and email are required' });
        }
...
module.exports = { register, login, verifyCode, forgotPassword, resetPassword, changePassword, updateProfile, refreshToken };